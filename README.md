...
**Поиск в шапке = поиск по артикулу CPMod.** Поле поиска отправляет AJAX-запрос
на эндпоинт модуля `{база}/search/{q}/` (параметры `CarModAjax/ShortResult/
ArtSearch`), показывает выпадающий список и обрабатывает `REDIRECT:` /
`SEARCH_NOT_FOUND` — как родной поиск модуля (`add/search/`). Базовый путь
настраивается: Customizer → «Шапка и блоки» → «Базовый путь модуля каталога»
(по умолчанию `/carparts`). Без JS форма уводит на страницу результатов
(модуль ловит `ArtSearch`). Папка `add/` в репозитории — копия модуля для
справки, в `.gitignore` (на проде поиск работает из `/carparts/add/`).
